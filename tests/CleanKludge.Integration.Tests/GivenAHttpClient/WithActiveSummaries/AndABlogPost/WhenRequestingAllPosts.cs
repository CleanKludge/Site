...
namespace CleanKludge.Integration.Tests.GivenAHttpClient.WithActiveSummaries.AndABlogPost
{
    [Ignore]
    [TestClass]
    public class WhenRequestingAllPosts
    {
        ...
        [TestMethod]
        public async Task ThenTheRequestIsSuccessful()
        {
            Assert.IsTrue(_result.IsSuccessStatusCode, await _result.Content.ReadAsStringAsync());
        }
    }
}